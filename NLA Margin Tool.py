...
def shift_nla_strips(context, threshold, delta):
    ...
    Алгоритм (чтобы не портить scale и не зажимать стрипы):
    
    Вправо (delta > 0):
      - Идём справа налево (по убыванию frame_start)
      - Для каждого стрипа: сначала frame_end += delta, затем frame_start += delta
    
    Влево (delta < 0):
      - Идём слева направо (по возрастанию frame_start)
      - Для каждого стрипа: сначала frame_start += delta, затем frame_end += delta
    
    После прохода: сбрасываем scale = 1.0 у всех сдвинутых стрипов.
    """
    if delta == 0:
        return

    # Собираем все стрипы которые нужно двигать
    to_move = []
    for strip in iter_scene_nla_strips(context):
        if strip.frame_start >= threshold:
    ...
    if delta > 0:
        # Двигаем вправо: справа налево, сначала frame_end потом frame_start
        to_move.sort(key=lambda s: s.frame_start, reverse=True)
        for strip in to_move:
            strip.frame_end += delta
            strip.frame_start += delta
    else:
        # Двигаем влево: слева направо, сначала frame_start потом frame_end
        to_move.sort(key=lambda s: s.frame_start, reverse=False)
        for strip in to_move:
            strip.frame_start += delta
            strip.frame_end += delta

    # Сбрасываем scale до 1.0 у всех сдвинутых стрипов
    for strip in to_move:
        strip.scale = 1.0
